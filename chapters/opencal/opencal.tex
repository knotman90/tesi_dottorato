...
to this specific sub-region, proved to be a simple but effective
...
included instead of the \texttt{OpenCAL} ones. For this purpose, it is
...
serial and OpenMP implementations of \texttt{OpenCAL} in terms of API and
internal algorithms.
...
main difference with respect to \texttt{OpenCAL} and \texttt{OpenCAL-OMP} is that an
\texttt{OpenCAL-CL} application is subdivided in two parts, one running on
...
and \texttt{OpenCAL-OMP} (cf. Algorithm \ref{algo:simuloop}), with the
exception that kernels are executed device-side and the simulation
process can not be currently, in this first \texttt{OpenCAL} release, made explicit.
...
considerations discussed for the \texttt{OpenCAL} and OpenCAL-OMP
implementations of the active cells optimization, even hold for the
\texttt{OpenCAL-CL} one. As a consequence, to both guaranty data integrity
...
computational complexity of the \texttt{OpenCAL-CL} parallel stream
compaction, as for the \texttt{OpenCAL} and \texttt{OpenCAL-OMP} implementations, is
...
\begin{figure}
	\begin{center}
		...
		\caption[\texttt{OpenCL} stream compaction algorithm adopted in \texttt{OpenCAL}.]{An example of application of the \texttt{OpenCAL-CL} parallel stream compaction algorithm. Active cells are represented in gray within a
...
\texttt{OpenCAL-CL} ones must have at least the \verb'__CALCL_MODEL_2D'
...
\lstset{language=[OpenCL]C,
	caption=Example of \texttt{OpenCAL-CL} kernel., 
...
An \texttt{OpenCAL-CL} host application is typically subdivided in the
following parts:
\begin{itemize}
...
The \texttt{OpenCAL-CL} host-side model definition does not differ from the
...
possible to use \texttt{OpenCAL} data types and functions from an
\texttt{OpenCAL-CL} host application.

\lstset{language=[OpenCL]C,
	caption={An example of \texttt{OpenCAL-CL} host-side application.}, 
...
\texttt{OpenCAL-CL} provides the \verb'CALCLManager' structure which,
...
\verb'CALCLManager' \texttt{OpenCAL-CL} data type, and initializes it
...
\lstset{language=[OpenCL]C,
	caption=Example of \texttt{OpenCAL-CL} access to platform and devices., 
	label=lst:CALOpenCL, 
	basicstyle=\footnotesize\ttfamily,
keywordstyle=\color{ultramarine}\ttfamily,
stringstyle=\color{rosemadder}\ttfamily,
...
\lstset{language=[OpenCL]C,
	...
	caption=The \texttt{OpenCAL-CL} kernel implementing the Conway's Game of Life elementary process.,
	basicstyle=\footnotesize\ttfamily,
	keywordstyle=\color{ultramarine}\ttfamily,
	stringstyle=\color{rosemadder}\ttfamily,
	commentstyle=\color{outerspace}\ttfamily,
...
file is included, together with the \texttt{OpenCAL} \verb'cal2DIO.h'
...
\lstset{language=[OpenCL]C,
	...
	caption=An \texttt{OpenCAL-CL} host-side implementation of the Conway's
	Game of Life.,
...
As for the case of the \texttt{OpenCAL} implementation of the Game of
...
In this section we show the OpenCAL, \texttt{OpenCAL-OMP} and OpenCAL-CL
...
According to OpenCL, the \texttt{OpenCAL-CL} implementation of
...
particular, the \texttt{OpenCAL-CL} implementation of $SciddicaT_{naive}$
...
defined by an \texttt{OpenCAL-CL} kernel as:
...
exploits the \texttt{OpenCAL} active cell optimization feature. The model is
...
Here we highlight the \texttt{OpenCAL} and \texttt{OpenCAL-OMP} differences between the $SciddicaT_{ac}$ and $SciddicaT_{naive}$ models. In particular, to properly exploit the active cells optimization, we have to change the definition of the CA object by using the \verb'CAL_OPT_ACTIVE_CELLS' parameter in the model
definition:
\lstset{language=[OpenCL]C,
	basicstyle=\footnotesize\ttfamily,
	keywordstyle=\color{ultramarine}\ttfamily,
...
\texttt{OpenCAL-CL} implementation of $SciddicaT_{ac}$, the host-side CA
...
\noindent As for the \texttt{OpenCAL} and \texttt{OpenCAL-OMP} implementations,
...
\texttt{OpenCAL-CL} implementation, while the $\sigma_3$ one is new, which
...
In this section we show the \texttt{OpenCAL} and \texttt{OpenCAL-OMP} implementations
...
In order to evaluate OpenCAL, \texttt{OpenCAL-OMP} and \texttt{OpenCAL-CL} from
...
time of the \texttt{OpenCAL} implementation of $SciddicaT_{naive}$) were
...
\texttt{OpenCAL-CL} currently does not support transparent management of the
...
In all cases, \texttt{OpenCAL} and \texttt{OpenCAL-OMP} benchmarks were executed on a
8-core/16 threads Intel Xeon 2.0GHz E5-2650 CPU based
workstation. One thread of the CPU was adopted for testing the
different \texttt{OpenCAL} versions of $SciddicaT$, while 2, 4, 8 and 16
threads were employed for benchmark experiments concerning
\texttt{OpenCAL-OMP} implementations. Moreover, two devices were adopted for
testing different versions of the \texttt{OpenCAL-CL} implementations of
...
the \texttt{OpenCAL} and \texttt{OpenCAL-OMP} different versions of $SciddicaT$ are
...
\begin{table}
	...
	\caption{Elapsed times (in seconds) registered for the simulation of the
		...
		different \texttt{OpenCAL} and \texttt{OpenCAL-OMP} versions of the $SciddicaT$
...
\begin{figure}
	\begin{center}
		...
		\caption[\textbf{Relative} speed-up obtained by the different \texttt{OpenCAL-OMP} versions of
		the $SciddicaT$ fluid-flow model.]{\textbf{Relative} speed-up obtained by the different \texttt{OpenCAL-OMP} versions of
...
\begin{figure}
	\begin{center}
		...
		\caption[\textbf{Absolute} speed-up obtained by the different \texttt{OpenCAL-OMP} versions of
		the $SciddicaT$ fluid-flow model.]{\textbf{Absolute} speed-up obtained by the different \texttt{OpenCAL-OMP} versions of
			the $SciddicaT$ fluid-flow model. The considered case study is the
			Tessina Landslide (cf. Figure \ref{fig:sciddicaT-simulation}). The
			adopted CPU was an Intel Xeon 2.0GHz E5-2650 CPU.}
...
The execution times of the benchmarks of the \texttt{OpenCAL-CL} different
...
\begin{table}
	...
	\caption{Elapsed times (in seconds) registered for the simulation of the
		Tessina Landslide (cf. Figure \ref{fig:sciddicaT-simulation}) by
		different \texttt{OpenCAL-CL} versions of the $SciddicaT$ fluid-flow model. The
...
\begin{figure}
	\begin{center}
		...
		\caption[\textbf{Relative} speed-up obtained by the different \texttt{OpenCAL-CL} versions of the
		$SciddicaT$ fluid-flow model.]{\textbf{Relative} speed-up obtained by the different \texttt{OpenCAL-CL} versions of the
...
\begin{figure}
	\begin{center}
		...
		\caption[\textbf{Absolute} speed-up obtained by the different \texttt{OpenCAL-CL} versions of the
		$SciddicaT$ fluid-flow model.]{\textbf{Absolute} speed-up obtained by the different \texttt{OpenCAL-CL} versions of the
			$SciddicaT$ fluid-flow model. The considered case study is the Tessina
			Landslide (cf. Figure \ref{fig:sciddicaT-simulation}). The adopted
			OpenCL compliant devices were a Nvidia Tesla K40 and an Nvidia GTX
			980.}
...
\texttt{OpenCAL} and \texttt{OpenCAL-OMP} different versions of $SciddicaT$ are
...
\begin{table}
	...
	\end{tabular}
	\caption[Elapsed times (in \si{s}) registered during the \emph{transition function stress test} by different \texttt{OpenCAL} and
	\texttt{OpenCAL-OMP} versions of the $SciddicaT$ fluid-flow model.]{Elapsed times (in \si{s}) registered during the \emph{transition function stress test} for the simulation of the Tessina Landslide
		(cf. Figure \ref{fig:sciddicaT-simulation}) by different \texttt{OpenCAL} and
		\texttt{OpenCAL-OMP} versions of the $SciddicaT$ fluid-flow model. The adopted
...
\begin{figure}
	\begin{center}
		...
		\caption[\textbf{Relative} speed-up obtained during the \emph{transition function stress	test} by the different \texttt{OpenCAL-OMP} versions of the $SciddicaT$ fluid-flow model.]{\textbf{Relative} speed-up obtained during the \emph{transition function stress
				test} by the different \texttt{OpenCAL-OMP} versions of the $SciddicaT$
...
\begin{figure}
	\begin{center}
		...
		\caption[\textbf{Absolute} speed-up obtained during the \emph{transition function stress test} by the different \texttt{OpenCAL-OMP} versions of the $SciddicaT$
		fluid-flow model.]{\textbf{Absolute} speed-up obtained during the \emph{transition function stress test} by the different \texttt{OpenCAL-OMP} versions of the $SciddicaT$
			fluid-flow model. The considered case study is the Tessina Landslide
			(cf. Figure \ref{fig:sciddicaT-simulation}). The adopted CPU was an
			Intel Xeon 2.0GHz E5-2650 CPU.}
...
The execution timings for the stress tests of the \texttt{OpenCAL-CL}
...
performance registered by the $SciddicaT$ \texttt{OpenCAL-CL} versions
...
\begin{figure}
	\begin{center}
		...
		\caption[\textbf{Relative} speed-up obtained during the \emph{transition function stress test} by different \texttt{OpenCAL-CL} versions of the $SciddicaT$ fluid-flow model.]{\textbf{Relative} speed-up obtained during the \emph{transition function stress
				test} by the different \texttt{OpenCAL-CL} versions of the $SciddicaT$
			fluid-flow model. The considered case study is the Tessina Landslide
...
\begin{figure}
	\begin{center}
		...
		\caption[\textbf{Absolute} speed-up obtained during the \emph{transition function stress test} by different \texttt{OpenCAL-CL} versions of the $SciddicaT$ fluid-flow model.]{Absolute speed-up obtained during the \emph{transition function stress
				test} by the different \texttt{OpenCAL-CL} versions of the $SciddicaT$
			fluid-flow model. The considered case study is the Tessina Landslide
			(cf. Figure \ref{fig:sciddicaT-simulation}). The adopted OpenCL
			compliant devices were a Nvidia Tesla K40 and an Nvidia GTX 980.}
...
\begin{table}
	...
	\end{tabular}
	\caption[Elapsed times obtained for the \emph{computational domain stress test} by different \texttt{OpenCAL} and \texttt{OpenCAL-OMP} versions
	of the $SciddicaT$ fluid-flow model.]{Elapsed times (in seconds) obtained for the \emph{computational domain stress test} based on the simulation shown in Figure \ref{fig:stresstestR}) by different \texttt{OpenCAL} and \texttt{OpenCAL-OMP} versions
...
\begin{figure}
	\begin{center}
		...
		\caption[\textbf{Relative} speed-up obtained during the \emph{computational domain stress test} by the different \texttt{OpenCAL-OMP} versions of the
		$SciddicaT$ fluid-flow model.]{\textbf{Relative} speed-up obtained during the \emph{computational domain
				stress test} by the different \texttt{OpenCAL-OMP} versions of the
...
the \texttt{OpenCAL-CL} different versions of $SciddicaT$ on the considered
...
\begin{table}
	...
	\ref{fig:stresstestR} by different \texttt{OpenCAL-CL} versions of the
...
\begin{figure}
	\begin{center}
		...
		\caption[\textbf{Relative} speed-up obtained during the \emph{computational domain
			stress test} by the different \texttt{OpenCAL-CL} versions of the $SciddicaT$
...
In this Chapter the first release of \texttt{OpenCAL} is presented, a new
...
were developed, namely \texttt{OpenCAL-OMP} and OpenCAL-CL, based on OpenMP
...
$A$. \texttt{OpenCAL-OMP} essentially implements the same strategy, even if a
...
\texttt{OpenCAL} and \texttt{OpenCAL-OMP} were designed to allow for the explicitation
...
\texttt{OpenCAL} implementation. Specifically, the \texttt{OpenCAL} and OpenCAL-OMP
...
algorithms will be performed in order to make \texttt{OpenCAL} still more
performing and MPI will be adopted to allow \texttt{OpenCAL} to exploit the
